...
//        AppIndex.AppIndexApi.end(client, viewAction);
    ...
    private void initView() {
        ...
        mMapView = (MapView) findViewById(R.id.rl_map);
        // 不显示缩放比例尺
            ...
            if (mRealPlayFullTalkAnimBtn != null) {
                ...
                mRealPlayFullOperateBar.post(new Runnable() {
                    ...
                    public void run() {
                        ...
                        mEndXy[0] = Utils.dip2px(PlayActivity.this, 20);
                        mEndXy[1] = mStartXy[1];
...
//        mRealPlaySv.setVisibility(View.VISIBLE);
    ...
    private void startVoiceTalk() {
        ...
        if (mOrientation == Configuration.ORIENTATION_LANDSCAPE) {
            ...
            startFullBtnAnim(mRealPlayFullAnimBtn, mStartXy, mEndXy, new AnimationListener() {
                ...
                @Override
                public void onAnimationEnd(Animation animation) {
                    Utils.showToast(PlayActivity.this, R.string.realplay_full_talk_start_tip);
                    mRealPlayFullTalkAnimBtn.setVisibility(View.VISIBLE);
                    mRealPlayFullAnimBtn.setVisibility(View.GONE);
                    onRealPlaySvClick();
                    //                    mFullscreenFullButton.setVisibility(View.VISIBLE);
    ...
    private void setQualityMode(final EZVideoLevel mode) {
        // 检查网络是否可用
        if (!ConnectionDetector.isNetworkAvailable(PlayActivity.this)) {
            // 提示没有连接网络
            Utils.showToast(PlayActivity.this, R.string.realplay_set_fail_network);
            return;
        }
...
//        mTalkPopupWindow.showAtLocation(parent, Gravity.BOTTOM, 0, 0);
            ...
            public void run() {
                if (mTalkRingView != null) {
                    mTalkRingView.setMinRadiusAndDistance(mTalkBackControlBtn.getHeight() / 2f,
                            Utils.dip2px(PlayActivity.this, 22));
                }
            }
        });
    }
    ...
    private void onRecordBtnClick() {
        ...
        if (!SDCardUtil.isSDCardUseable()) {
            // 提示SD卡不可用
            Utils.showToast(PlayActivity.this, R.string.remoteplayback_SDCard_disable_use);
            return;
        }

        if (SDCardUtil.getSDCardRemainSize() < SDCardUtil.PIC_MIN_MEM_SPACE) {
            // 提示内存不足
            Utils.showToast(PlayActivity.this, R.string.remoteplayback_record_fail_for_memory);
            return;
        }
    ...
    private void stopRealPlayRecord() {
        ...
        Toast.makeText(PlayActivity.this, getResources().getString(R.string.already_saved_to_volume), Toast.LENGTH_SHORT).show();
    ...
    private void    onCapturePicBtnClick() {
        ...
        if (SDCardUtil.getSDCardRemainSize() < SDCardUtil.PIC_MIN_MEM_SPACE) {
            // 提示内存不足
            Utils.showToast(PlayActivity.this, R.string.remoteplayback_capture_fail_for_memory);
            return;
        }
                ...
                public void run() {
                    ...
                    if (bmp != null) {
                        try {
                            ...
                            MediaScanner mMediaScanner = new MediaScanner(PlayActivity.this);
                            mMediaScanner.scanFile(path, "jpg");
                            runOnUiThread(new Runnable() {
                                @Override
                                public void run() {
                                    Toast.makeText(PlayActivity.this, getResources().getString(R.string.already_saved_to_volume)+path, Toast.LENGTH_SHORT).show();
                                }
                            });
...
//    }
    ...
    private void startRealPlay() {
        ...
        mEZPlayer = App.getOpenSDK().createPlayer("C23702836", 1);
        mEZPlayer.setHandler(mHandler);
        mEZPlayer.setSurfaceHold(mRealPlaySh);
        mEZPlayer.setPlayVerifyCode("OPLYPV");
    ...
    private void handlePtzControlFail(Message msg) {
        ...
        switch (msg.arg1) {
            case ErrorCode.ERROR_CAS_PTZ_CONTROL_CALLING_PRESET_FAILED:// 正在调用预置点，键控动作无效
                Utils.showToast(PlayActivity.this, R.string.camera_lens_too_busy, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_PRESETING_FAILE:// 当前正在调用预置点
                Utils.showToast(PlayActivity.this, R.string.ptz_is_preseting, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_CONTROL_TIMEOUT_CRUISE_TRACK_FAILED:// 键控动作超时(当前正在轨迹巡航)
                Utils.showToast(PlayActivity.this, R.string.ptz_control_timeout_cruise_track_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_INVALID_POSITION_FAILED:// 当前预置点信息无效
                Utils.showToast(PlayActivity.this, R.string.ptz_preset_invalid_position_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_CURRENT_POSITION_FAILED:// 该预置点已是当前位置
                Utils.showToast(PlayActivity.this, R.string.ptz_preset_current_position_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRESET_SOUND_LOCALIZATION_FAILED:// 设备正在响应本次声源定位
                Utils.showToast(PlayActivity.this, R.string.ptz_preset_sound_localization_failed, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_MIRRORING_FAILED:// 设备正在镜像操作（设备镜像要几秒钟，防止频繁镜像操作）
                Utils.showToast(PlayActivity.this, R.string.ptz_operation_too_frequently, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_PRESET_EXCEED_MAXNUM_FAILED:// 当前预置点超过最大个数
                Utils.showToast(PlayActivity.this, R.string.ptz_preset_exceed_maxnum_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_PRIVACYING_FAILED:// 设备处于隐私遮蔽状态（关闭了镜头，再去操作云台相关）
                Utils.showToast(PlayActivity.this, R.string.ptz_privacying_failed, msg.arg1);
                break;
            case ErrorCode.ERROR_CAS_PTZ_TTSING_FAILED:// 设备处于语音对讲状态(区别以前的语音对讲错误码，云台单独列一个）
                Utils.showToast(PlayActivity.this, R.string.ptz_mirroring_failed, msg.arg1);
                break;
            ...
            case ErrorCode.ERROR_CAS_PTZ_ROTATION_RIGHT_LIMIT_FAILED:// 设备云台旋转到达右限位
                ...
                break;
            default:
                Utils.showToast(PlayActivity.this, R.string.ptz_operation_failed, msg.arg1);
                break;
        }
    }
...
/*
    ...
    private void handleVoiceTalkFailed(ErrorInfo errorInfo) {
        ...
        switch (errorInfo.errorCode) {
            case ErrorCode.ERROR_TRANSF_DEVICE_TALKING:
                Utils.showToast(PlayActivity.this, R.string.realplay_play_talkback_fail_ison);
                break;
            case ErrorCode.ERROR_TRANSF_DEVICE_PRIVACYON:
                Utils.showToast(PlayActivity.this, R.string.realplay_play_talkback_fail_privacy);
                break;
            case ErrorCode.ERROR_TRANSF_DEVICE_OFFLINE:
                Utils.showToast(PlayActivity.this, R.string.realplay_fail_device_not_exist);
                break;
            ...
            case ErrorCode.ERROR_TTS_HNADLE_TIMEOUT:
                Utils.showToast(PlayActivity.this, R.string.realplay_play_talkback_request_timeout, errorInfo.errorCode);
                break;
            ...
            case ErrorCode.ERROR_CAS_AUDIO_SEND_ERROR:
                Utils.showToast(PlayActivity.this, R.string.realplay_play_talkback_network_exception, errorInfo.errorCode);
                break;
            default:
                Utils.showToast(PlayActivity.this, R.string.realplay_play_talkback_fail, errorInfo.errorCode);
                break;
        }
    }
    ...
    private void handleSetVedioModeFail(int errorCode) {
        ...
        Utils.showToast(PlayActivity.this, R.string.realplay_set_vediomode_fail, errorCode);
    }
    ...
    private void handleRecordFail() {
        Utils.showToast(PlayActivity.this, R.string.remoteplayback_record_fail);
        if (mIsRecording) {
            stopRealPlayRecord();
        }
    }
    ...
    private void updateRealPlayFailUI(int errorCode) {
        ...
        switch (errorCode) {
            case ErrorCode.ERROR_TRANSF_ACCESSTOKEN_ERROR:
                ActivityUtils.goToLoginAgain(PlayActivity.this);
                return;
...
//                SecureValidate.secureValidateDialog(this, this);
    ...
    private void showType() {
        if (Config.LOGGING && mEZPlayer != null) {
            Utils.showLog(PlayActivity.this, "getType " + ",取流耗时：" + (mStopTime - mStartTime));
        }
    }
